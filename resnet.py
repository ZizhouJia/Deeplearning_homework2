...
class ResNet(nn.Module):
    ...
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=65, zero_init_residual=False,
        ...
        self.conv_features=nn.Sequential(nn.Conv2d(2048,1024,4,1,0,bias=False),nn.ReLU(inplace=True),nn.Dropout(p=0.5))
        # nn.Conv2d(256,2048,7,1,0,bias=False),nn.ReLU(inplace=True))
        self.dropout=nn.Dropout(p=0.5)


        self.fc = nn.Linear(1024, num_classes*4)
    ...
    def forward(self, x):
        ...
        x2 = self.layer4(x)
        x = self.layer5(x2)
        x2=torch.mean(x2.view(x.size(0),x.size(1),-1),dim=2)
        x2=self.fc2(x2)
